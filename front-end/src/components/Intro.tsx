export function Intro() {
  return (
    <>
      <h1 className="tight mt-10 font-display text-4xl font-light text-white">
        <span className="text-sky-300">Board of Supervisors</span>
        <br />
        Dean Preston <br />
      </h1>
      <p className="mt-4 text-sm/6 text-gray-300">
        Supervisor Dean Preston of SF's District 5 is a Democratic Socialist and
        tenant rights attorney. With 20 years in the affordable housing
        movement, he fought for rent control, formed a tenant rights group, and
        authored a law for tenant legal representation. He champions community
        engagement, supports small businesses, and seeks systemic change.
      </p>
    </>
  )
}
